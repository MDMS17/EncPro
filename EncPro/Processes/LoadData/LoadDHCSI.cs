...
namespace EncPro.LoadData
{
    ...
    {
        public static async Task LoadDHCSI(string StartDate, string EndDate, int PageNumber)
        {
            List<MeditracHeader> cacheHeaders = await MeditracUtility.GetMeditracHeaders("'305','306'", "UB", StartDate, EndDate, "'TBP','ENC'", PageNumber);
            List<MeditracLine> cacheLines = await MeditracUtility.GetMeditracLines("'305','306'", "UB", StartDate, EndDate, "'TBP','ENC'", PageNumber);
            List<MeditracCode> CacheCodes = await MeditracUtility.GetMeditracCodes("'305','306'", "UB", StartDate, EndDate, "'TBP','ENC'", PageNumber);
