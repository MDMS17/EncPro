...
namespace EncPro.LoadData
{
    ...
    {
        public static async Task LoadCCII(string StartDate, string EndDate, int PageNumber)
        {
            List<MeditracHeader> cacheHeaders = await MeditracUtility.GetMeditracHeaders("'810','812'", "UB", StartDate, EndDate, "'TBP','ENC','CLD'", PageNumber);
            List<MeditracLine> cacheLines = await MeditracUtility.GetMeditracLines("'810','812'", "UB", StartDate, EndDate, "'TBP','ENC','CLD'", PageNumber);
            List<MeditracCode> CacheCodes = await MeditracUtility.GetMeditracCodes("'810','812'", "UB", StartDate, EndDate, "'TBP','ENC','CLD'", PageNumber);
            ...
            foreach (MeditracHeader cacheHeader in cacheHeaders)
            {
                ...
                {
                    GlobalVariables.TotalCCII--;
                    continue;
                }
                ClaimHeader header = new ClaimHeader();
                header.FileID = 0;
                header.ClaimID = cacheHeader.GroupNumber + DateTime.Now.ToString("yyyyMMddHH") + "I" + PageNumber.ToString().PadLeft(2, '0') + sequenceNumber.ToString().PadLeft(4, '0');
