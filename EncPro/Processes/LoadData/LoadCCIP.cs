...
{
    public partial class LoadMeditrac
    {
        public static async Task LoadCCIP(string StartDate, string EndDate, int PageNumber)
        {
            List<MeditracHeader> cacheHeaders = await MeditracUtility.GetMeditracHeaders("'810','812'", "HCF", StartDate, EndDate, "'TBP','ENC','CLD'", PageNumber);
            List<MeditracLine> cacheLines = await MeditracUtility.GetMeditracLines("'810','812'", "HCF", StartDate, EndDate, "'TBP','ENC','CLD'", PageNumber);
            List<MeditracCode> CacheCodes = await MeditracUtility.GetMeditracCodes("'810','812'", "HCF", StartDate, EndDate, "'TBP','ENC','CLD'", PageNumber);
            ...
            foreach (MeditracHeader cacheHeader in cacheHeaders)
            {
                ...
                {
                    GlobalVariables.TotalCCIP--;
                    continue;
                }
                //check procedure codes, if DME, clone
                if (cacheLines.Where(x => x.ClaimId == cacheHeader.ClaimId).Count(y => GlobalVariables.DMEProcCodes.Contains(y.ProcedureCode)) > 0)
                {
                    MeditracHeader DMEHeader = cacheHeader;
                    List<MeditracLine> DMELines = cacheLines.Where(x => x.ClaimId == cacheHeader.ClaimId).Where(y => GlobalVariables.DMEProcCodes.Contains(y.ProcedureCode)).ToList();
                    ClaimHeader dmeHeader = new ClaimHeader();
                    dmeHeader.FileID = 0;
                    dmeHeader.ClaimID = DMEHeader.GroupNumber + DateTime.Now.ToString("yyyyMMddHH") + "E" + PageNumber.ToString().PadLeft(2, '0') + sequenceNumber.ToString().PadLeft(4, '0');
                ...
                if (cacheLines.Where(x => x.ClaimId == cacheHeader.ClaimId).Count(y => !GlobalVariables.DMEProcCodes.Contains(y.ProcedureCode)) > 0)
                {
                    ...
                    header.FileID = 0;
                    header.ClaimID = cacheHeader.GroupNumber + DateTime.Now.ToString("yyyyMMddHH") + "P" + PageNumber.ToString().PadLeft(2, '0') + sequenceNumber.ToString().PadLeft(4, '0');
                    ...
                    foreach (MeditracLine cacheLine in cacheLines.Where(x => x.ClaimId == cacheHeader.ClaimId).Where(y => !GlobalVariables.DMEProcCodes.Contains(y.ProcedureCode)).OrderBy(x => x.LineNumber))
                    {
                        decimal lineChargeAmount = 0, linePaidAmount = 0, lineCopayAmount = 0, lineCoinsuranceAmount = 0, lineDeductAmount = 0, lineCOBPaidAmount = 0;
            ...
            {
                context.ClaimCAS.AddRange(cases);
                context.ClaimCRCs.AddRange(crcs);
                context.ClaimHeaders.AddRange(headers);
                context.ClaimHIs.AddRange(his);
                context.ClaimK3s.AddRange(k3s);
                context.ClaimLineSVDs.AddRange(lineSvds);
                context.ClaimNtes.AddRange(notes);
                context.ClaimProviders.AddRange(providers);
                context.ClaimPWKs.AddRange(pwks);
                context.ClaimSBRs.AddRange(sbrs);
                context.ClaimSecondaryIdentifications.AddRange(sids);
                context.ServiceLines.AddRange(lines);
