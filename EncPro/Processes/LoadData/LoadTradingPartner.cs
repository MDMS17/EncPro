...
{
    public class LoadTradingPartner
    {
        public static async Task<string> LoadTradingPartner837(FileInfo fi, string SourceFolder, string ArchiveFolder, string EVRFolder)
        {
            ...
            using (var context = new SubcacheContext())
            {
                ...
                {
                    //listBox1.Items.Add("File " + fi.Name + " already processed before");
                ...
                if (encounterCount == 0)
                {
                    //listBox1.Items.Add("File " + fi.Name + " not valid");
                    sbLog.AppendLine("File " + fi.Name + " not valid");
                    return "";
                }
                //listBox1.Items.Add("Processing file " + fi.Name + " total records: " + encounterCount.ToString());
                        ...
                        case "HL":
                            ...
                            switch (segments[3])
                            {
                                case "20":
                                    if (saveFlag)
                                    {
                                        ParseData.Process837.InitilizeClaim("BillingProvider", ref claim, ref claims, ref submittedFile);
                                        saveFlag = false;
                                    }
                                    LoopName = "2000A";
                                    provider = new ClaimProvider();
                                ...
                                case "22":
                                    if (saveFlag)
                                    {
                                        ParseData.Process837.InitilizeClaim("Subscriber", ref claim, ref claims, ref submittedFile);
                                        saveFlag = false;
                                    }
                                    LoopName = "2000B";
                                    sbr = new ClaimSBR();
                                ...
                                case "23":
                                    if (saveFlag)
                                    {
                                        ParseData.Process837.InitilizeClaim("Patient", ref claim, ref claims, ref submittedFile);
                                        saveFlag = false;
                                    }
                                    LoopName = "2000C";
                ...
                ParseData.Process837.InitilizeClaim("Claim", ref claim, ref claims, ref submittedFile);
                List<ClaimValidationError> errors = new List<ClaimValidationError>();
                int RecordCount = claims.Select(x => x.Header).Count();

                //validation, check member eligibility (90)
                ...
                foreach (Claim c1 in claims)
                {
                    ...
                    List<MemberDetail> memberDetails = await MeditracUtility.GetMemberDetails(memberNumber, serviceDate);
                ...
                if (submittedFile.FileType == "005010X223A2") //institutional
                {
                    ...
                    foreach (Claim c1 in claims.Where(x => x.Header.ExceptionCode != "90"))
                    {
                        foreach (ServiceLine l1 in c1.Lines)
                        {
                            string cin = allMembers.Where(x => x.ClaimId == c1.Header.ClaimID).Select(x => string.IsNullOrEmpty(x.CIN) ? x.HIC : x.CIN).First();
                ...
                //save valid claims to database
                foreach (var c in claims.Where(x => x.Header.ExceptionCode == "90" || x.Header.ExceptionCode == "53" || x.Header.ExceptionCode == "52" || x.Header.ExceptionCode == "63").ToList()) claims.Remove(c);

                //enrich, add county code to claimid, add necessary segments like NTE
                foreach (Claim c1 in claims)
                {
                ...
                //generate EVR for trading partners
                CalculatedCounts evrCount = new CalculatedCounts();
                evrCount.ValidationErrors = errors;
                evrCount.RecordCount = RecordCount;
                evrCount.ProcessedCount = RecordCount;
                evrCount.TransmissionmName = submittedFile.FileName;
                evrCount.RejectedCount = RecordCount - evrCount.ProcessedCount;
                evrCount.Now = DateTime.Now;
                evrCount.ExemptDuplicateCount = errors.Count(x => LoadData.LoadEVR.ExemptDuplicated.Contains(x.ErrorId));
                evrCount.ExemptMemberNotEligibleCount = errors.Count(x => LoadData.LoadEVR.ExemptMemberNotEligible.Contains(x.ErrorId));
                evrCount.ExemptInProgressCount = errors.Count(x => LoadData.LoadEVR.ExemptInProgress.Contains(x.ErrorId));
                ...
                evrCount.ValidCount = claims.Select(x => x.Header).Count();
                evrCount.Validity = evrCount.ValidCount / (float)evrCount.EligibileForIehpEditChecks * 100;
                File.WriteAllText(Path.Combine(EVRFolder, Path.GetFileNameWithoutExtension(fi.Name) + "_EVR.txt"), LoadEVR.EVRReport(evrCount));
