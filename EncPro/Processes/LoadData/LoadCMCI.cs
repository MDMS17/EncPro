...
namespace EncPro.LoadData
{
    ...
    {
        public static async Task LoadCMCI(string StartDate, string EndDate, int PageNumber)
        {
            List<MeditracHeader> cacheHeaders = await MeditracUtility.GetMeditracHeaders("'H53'", "UB", StartDate, EndDate, "'TBP','ENC','CLD'", PageNumber);
            List<MeditracLine> cacheLines = await MeditracUtility.GetMeditracLines("'H53'", "UB", StartDate, EndDate, "'TBP','ENC','CLD'", PageNumber);
            List<MeditracCode> CacheCodes = await MeditracUtility.GetMeditracCodes("'H53'", "UB", StartDate, EndDate, "'TBP','ENC','CLD'", PageNumber);
