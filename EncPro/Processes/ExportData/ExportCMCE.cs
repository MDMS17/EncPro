...
namespace EncPro
{
    ...
    {
        public static async Task ExportCMCE(string CountyCode, string flag)
        {
            ...
            for (int i = 0; i <= pages; i++)
            {
                ...
                if (headers.Count > 0)
                {
                    ...
                    File.WriteAllText(Path.Combine(DestinationFolder, "REYW480.XEJY.MEDS.DME_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".H5355.DCA006.P"), sb.ToString());
                }
            }
            context.Dispose();
        }
