...
namespace EncPro
{
    ...
    {
        public static async Task ExportCCIE(string CountyCode, string flag)
        {
            string DestinationFolder = ConfigurationManager.AppSettings["DestinationFolder"];
            SubcacheContext context = new SubcacheContext();
            int pages = context.ClaimHeaders.Count(x => x.ExportType == "ECMS" + CountyCode) / 5000;
            ...
            for (int i = 0; i <= pages; i++)
            {
                ...
                SegmentCount = 0;
                HLID = 1;
                List<ClaimHeader> headers = await context.ClaimHeaders.Where(x => x.ExportType == "ECMS" + CountyCode).OrderBy(x => x.ID).Skip(i * 5000).Take(5000).ToListAsync();
                if (headers.Count > 0)
                {
                    ...
                    File.WriteAllText(Path.Combine(DestinationFolder, "REYW480.XEJY.MMCD.DME_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".H5355.DCA006.P"), sb.ToString());
                }
            }
            context.Dispose();
        }
