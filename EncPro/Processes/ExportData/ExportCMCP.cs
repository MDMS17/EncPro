...
namespace EncPro
{
    ...
    {
        public static async Task ExportCMCP(string CountyCode, string flag)
        {
            string DestinationFolder = ConfigurationManager.AppSettings["DestinationFolder"];
            SubcacheContext context = new SubcacheContext();
            int pages = context.ClaimHeaders.Count(x => x.ExportType == "PCMS" + CountyCode) / 5000;
            ...
            for (int i = 0; i <= pages; i++)
            {
                ...
                if (headers.Count > 0)
                {
                    ...
                    File.WriteAllText(Path.Combine(DestinationFolder, "REYW480.XEJY.MEDS.837P_" + DateTime.Now.ToString("yyyyMMddHHmmss") + ".H5355.DCA006.P"), sb.ToString());
                }
            }
            context.Dispose();
        }
