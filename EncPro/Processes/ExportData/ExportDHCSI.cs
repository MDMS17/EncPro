...
namespace EncPro
{
    ...
    {
        public static async Task Export837IAsync(string CountyCode, string flag)
        {
            string DestinationFolder = ConfigurationManager.AppSettings["DestinationFolder"];
            SubcacheContext context = new SubcacheContext();
            int pages = context.ClaimHeaders.Count(x => x.ExportType == "IDHCS" + CountyCode) / 5000;
            ...
            for (int i = 0; i <= pages; i++)
            {
                ...
                List<ClaimHeader> headers = await context.ClaimHeaders.Where(x => x.ExportType == "IDHCS" + CountyCode).OrderBy(x => x.ID).Skip(i * 5000).Take(5000).ToListAsync();
                if (headers.Count > 0)
                {
                    context.Database.ExecuteSqlCommand("truncate table Sub_History.ClaimTempHost");
